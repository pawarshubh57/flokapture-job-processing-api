...
{
    public class CobolProecssingFinalVersionController : ApiController
    {
        ...
        private readonly CobolProcessingFinalVersionHelper cobolProcessingFinalVersionHelper = new CobolProcessingFinalVersionHelper();
        public CobolProecssingFinalVersionController()
        {
        }

        public CobolProecssingFinalVersionController(ICodeVortoService codeVortoService)
        {
        ...
        public async Task<HttpResponseMessage> StartCobolProcessing(int projectId)
        {
            using (_codeVortoService = new CodeVortoService())
            {
                ...
                var ignoredFile =
                    await
                        projectMasterRepository.GetAllIgnoredFiles<IgnoreFiles>(projectDetails.LanguageId)
            ...
            using (_codeVortoService = new CodeVortoService())
            {
                try
                {
                    ...
                    foreach (var keyValue in methodsLines)
                    {
                        ...
                        var lastIndexOfStopRun = programFileLines.FindLastIndex(s => s == "STOP RUN" || s == "STOP RUN.");
                        ...
                        if (mainListBlock.Any(x => x == "STOP RUN" || x == "STOP RUN."))
                        {
                    ...
                    foreach (var line in programFileLines)
                    {
                        ...
                        */
                        ...
                        if (callExternal.Any(l => line == l.StartIndicator))
                        {
                            var stmtReferenceMaster = new StatementReferenceMaster
                            {
                                FileId = fileMaster.FileId,
                        ...
                        if (line == "END" || line == "STOP RUN" || line == "STOP RUN.")
                        {
                            var stmtReferenceMaster = new StatementReferenceMaster
                            {
                                FileId = fileMaster.FileId,
            ...
            using (_codeVortoService = new CodeVortoService())
            {
                try
                {
                    ...
                    foreach (var statement in statementReferMaster)
                    {
                        ...
                        var regexProc = @"(EXEC\s*PROC\s*=)";
                        var regexPgm = @"(EXEC\s*PGM\s*=)";
                        ...
                        else if (Regex.IsMatch(originalStatement, regexPgm))
                        {
                            ...
                            if (!string.IsNullOrEmpty(pgmName))
                            {
                                var pName =
                                    copyOfFileMaster.ToList().Where(f => !string.IsNullOrEmpty(pgmName)
                                                                         && f.FileName.StartsWith(pgmName) && f.FileTypeExtensionId == 6).ToList();
                                if (!pName.Any()) continue;
                                var projectDetails1 =
                                    _codeVortoService.ProjectMasterRepository.GetItem(pName[0].ProjectId);
                                var pNameNew = projectDetails1.ProjectName;
                    ...
                    string sqlQuery =
                        "SELECT * FROM statementreferencemaster WHERE ProjectId = 30 " +
                    ...
                    foreach (var statement in genericBlocks)
                    {
                        ...
                        var actionWorkflow = new ActionWorkflows
                        {
                            ...
                            OriginObject =
                                ...
                                fileMaster.FilePath.Replace(".jcl", "")
                                    .Replace(projectMaster.PhysicalPath, "")
                                    .Trim()
                                    .Replace("\\", "."),
       ...
       "========================================================================================");
        ...
        public async Task<IHttpActionResult> GetWorkFlowWorkSpaceParallel(int projectId, int stmtId)
        {
         ...
         "========================================================================================");
                ...
                var _appDbContext = _codeVortoService.AppDbContextRepository;
                ...
                foreach (var treeItem in copyOfLstTreeView)
                {
                    ...
                    var anyFile = copyOfFileMaster.ToList()
                        .Any(f => (programName != null)
                                  ...
                                  && (f.FileTypeExtensionId == 9) && f.SolutionId == solutionId);
        ...
        public async Task<IHttpActionResult> GetWorkFlowWorkSpace(int projectId, string className, string methodName, int actionWorkflowId)
        {
            using (_codeVortoService = new CodeVortoService())
            {
                ...
                string qryMaxTreeNodes =
                    "select * from   where PropertyName= 'Max Tree Nodes' ";
